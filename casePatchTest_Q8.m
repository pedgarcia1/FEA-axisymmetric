...
pressureNormal = 100;
a = 1; b = 2;  h = 1;
nElementsZ = 2; nElementsR = 2;  distorsion = 0.1;   
...
s(1) = scatter(nodes(boundaryConditionsArray(:,1),1),nodes(boundaryConditionsArray(:,1),2),'filled','red');
s(2) = scatter(nodes(boundaryConditionsArray(:,2),1),nodes(boundaryConditionsArray(:,2),2),'filled','green');
...
switch caso
    ...
    case 2
        % pointLoadsArray = distributedLoad_3(elementType,Lside,1,pointLoadsArray,nodes,pressureNormal);
        % pointLoadsArray = distributedLoad_3(elementType,Rside,1,pointLoadsArray,nodes,-pressureNormal);
        pointLoadsArray1 = -2*pi*DistributiveLoads([pressureNormal 0],elementType,elements,nodes,'Left',sideNodes);
        pointLoadsArray2 = 2*pi*DistributiveLoads([pressureNormal 0],elementType,elements,nodes,'Right',sideNodes);
        pointLoadsArray = pointLoadsArray1 + pointLoadsArray2;
    case 3
        % pointLoadsArray = distributedLoad_3(elementType,sideNodes(3,:),2,pointLoadsArray,nodes,-pressureNormal);
        pointLoadsArray = 2*pi*DistributiveLoads([0 pressureNormal],elementType,elements,nodes,'Up',sideNodes);
end
s(3) = scatter(nodes(pointLoadsArray(:,1) ~= 0,1),nodes(pointLoadsArray(:,1) ~= 0,2),'filled','yellow');
...
meshPlot(elements,nodes+magnificationFactor*reshape(displacementsVector,nDimensions,nNodes)','b','No');
bandPlot(elements,nodes+magnificationFactor*reshape(displacementsVector,nDimensions,nNodes)',squeeze(elementStressExtrapolated(:,:,2)));
subplot(2,2,3); title('\sigma_{z}','Interpreter','tex')
meshPlot(elements,nodes+magnificationFactor*reshape(displacementsVector,nDimensions,nNodes)','b','No');
bandPlot(elements,nodes+magnificationFactor*reshape(displacementsVector,nDimensions,nNodes)',squeeze(elementStressExtrapolated(:,:,3)));
subplot(2,2,4); title('\tau_{zr}','Interpreter','tex')
meshPlot(elements,nodes+magnificationFactor*reshape(displacementsVector,nDimensions,nNodes)','b','No');
bandPlot(elements,nodes+magnificationFactor*reshape(displacementsVector,nDimensions,nNodes)',squeeze(elementStressExtrapolated(:,:,4)));
