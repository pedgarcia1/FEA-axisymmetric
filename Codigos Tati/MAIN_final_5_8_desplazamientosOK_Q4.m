...
%% Mesh generation
 [meshInfo.elements,meshInfo.nodes,meshInfo.vertexNodes,meshInfo.sideNodes]=quadrilateralDomainMeshGenerator(eleType,'Straight',4000,1000,20,10,0,0);
%  
 meshInfo.nodes(:,1)=meshInfo.nodes(:,1)+1000; %Axisimetrico respecto de X
...
[constitutive] = constitutiveIsotropicMatrix('Axisymmetric',200000,0);
...
% pointLoadsArray(meshInfo.sideNodes(2,meshInfo.sideNodes(2,:)>0),1)=-p0;
...
if keyPlots
    ...
    if keyPlots2
        for iNode=1:nNodes
            ...
            quiver(meshInfo.nodes(iNode,1),meshInfo.nodes(iNode,2), pointLoadsArray(iNode, 1), pointLoadsArray(iNode, 2), 'color', 'k', 'linewidth', 1.5, 'maxheadsize', 0.5)
        end
    end
end
